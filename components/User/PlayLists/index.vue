<script setup lang="ts">
  import { EllipsisVertical, HeartIcon, Pencil, PlusIcon, Share2Icon } from 'lucide-vue-next';
  import { ref } from 'vue';
  ...
  <div>
    <div v-if="playlists && playlists.length" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
      <div v-for="playlist in playlists" :key="playlist.id">
        <NuxtLink :to="`/dashboard/playlists/${playlist.id}`" class="flex items-center bg-zinc-900 p-4 hover:bg-zinc-950 cursor-pointer relative">
          <div class="flex-grow min-w-0 flex items-center space-x-4">
            ...
            <div v-if="activeDropdown === playlist.id" class="absolute z-20 top-14 right-0 mt-2 w-48 rounded-md shadow-lg bg-zinc-800 ring-1 ring-black ring-opacity-5">
              <div class="py-1" role="menu" aria-orientation="vertical" aria-labelledby="options-menu">
                <a href="#" class="flex items-center gap-2 px-4 py-2 text-sm text-white hover:bg-zinc-700" role="menuitem">
                  <component :is="PlusIcon" class="w-4 h-4 text-white cursor-pointer" />
                  Add item
                </a>
                <a href="#" class="flex items-center gap-2 px-4 py-2 text-sm text-white hover:bg-zinc-700" role="menuitem">
                  <component :is="Pencil" class="w-4 h-4 text-white cursor-pointer" />
                  Edit PlayList
                </a>
                <a href="#" class="flex items-center gap-2 px-4 py-2 text-sm text-white hover:bg-zinc-700" role="menuitem">
                  <component :is="Share2Icon" class="w-4 h-4 text-white cursor-pointer" />
                  Share PlayList
                </a>
                <a href="#" class="flex items-center gap-2 px-4 py-2 text-sm text-white hover:bg-zinc-700" role="menuitem">
                  <component :is="HeartIcon" class="w-4 h-4 text-white cursor-pointer" />
                  Like PlayList
                </a>
              </div>
            </div>
