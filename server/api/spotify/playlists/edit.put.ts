...
const updatePlayList = async (accessToken: string, playlistId: string, playlistName: string, playlistDescription: string, isPublic: boolean) => {
  try {
...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    return await updatePlayList(accessToken, profile.id, name, description, isPublic);
  } catch (error: any) {
    if (error.message === 'The access token expired') {
      const data = await refreshAccessToken(refreshToken);
      setAuthCookies(event, data);
      const profile = await getSpotifyProfile(data.access_token);
      return await updatePlayList(data.access_token, profile.id, name, description, isPublic);
    }
    throw error;
  }
});
